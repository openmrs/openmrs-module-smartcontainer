...
public class LabResultController extends BaseSmartController<LabResultRDFSource> {
	
	private static final Log log = LogFactory.getLog(LabResultController.class);
	
	@RequestMapping("records/{pid}/lab_results")
	public ModelAndView handle(@PathVariable("pid") Patient patient, HttpServletResponse resp) {
		if (log.isDebugEnabled())
			log.debug("In the LabResult Controller");
		resp.setContentType("text/xml"); // actually I use a constant
		Writer writer;
		try {
			writer = resp.getWriter();
			List<SmartLabResult> labs = Context.getService(SmartDataService.class).getAllForPatient(patient,
			    SmartLabResult.class);
			writer.write(getResource().getRDF(labs));
			writer.close();
		}
		catch (IOException e) {
