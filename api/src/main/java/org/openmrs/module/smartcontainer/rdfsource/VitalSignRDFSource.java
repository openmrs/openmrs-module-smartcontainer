...
public class VitalSignRDFSource extends RdfSource {
		...
		for (SmartVitalSigns s : signs) {
			...
			*/
			...
			if (s.getBloodPressure() != null) {
				...
				if (s.getBloodPressure().getDiastolic() != null) {
					...
					graph.handleStatement(factory.createStatement(bpNode, diastolic,
					    addVitalsign(s.getBloodPressure().getDiastolic(), graph)));
				}
				/*Add child node to Blood Pressure node
				     * <sp:systolic>
				     *  ..Vital Sign node
				...
				if (s.getBloodPressure().getSystolic() != null) {
					...
					graph.handleStatement(factory.createStatement(bpNode, systolic,
					    addVitalsign(s.getBloodPressure().getSystolic(), graph)));
				}
			}
			...
			if (s.getWeight() != null) {
				...
				graph.handleStatement(factory.createStatement(vitalSignNode, weight, addVitalsign(s.getWeight(), graph)));
			}
			/*Add child node to parent node
			* <sp:height>
			*  ..Vital Sign node
			...
			*/
			if (s.getHeight() != null) {
				...
				graph.handleStatement(factory.createStatement(vitalSignNode, height, addVitalsign(s.getHeight(), graph)));
			}
			/*Add child node to parent node
			* <sp:bodyMassIndex>
			*  ..Vital Sign node
			...
			*/
			if (s.getOxygenSaturation() != null) {
				URI oxygenSaturation = factory.createURI(sp, "oxygenSaturation");
	 ...
	 * <p/>
	 * <sp:Encounter> ..child nodes </sp:Encounter>
	 * 
	 ...
	 * @throws IOException
	 */
	private Value addEncounter(SmartEncounter smartEncounter, RDFXMLWriter graph) throws IOException, RDFHandlerException {
		/*Add parent node
		   * <sp:Encounter>
		   * ..child nodes
		   * </sp:Encounter>
		...
		if (smartEncounter.getStartDate() != null) {
			...
			Value startDateVal = factory.createLiteral(smartEncounter.getStartDate());
		...
		if (smartEncounter.getEndDate() != null) {
			...
			Value endDateVal = factory.createLiteral(smartEncounter.getEndDate());
		...
		graph.handleStatement(factory.createStatement(encounterNode, encounterType,
		    RdfUtil.codedValue(factory, graph, smartEncounter.getEncounterType())));
	 ...
	 * Create following rdf:
	 * <p/>
	 * <sp:VitalSign> ..child node </sp:VitalSign>
	 * 
	 * @param v
	 * @param graph
	 * @return
	 * @throws IOException
	 */
	private BNode addVitalsign(VitalSign v, RDFXMLWriter graph) throws IOException, RDFHandlerException {
		/*
		   * Add parent node
		   * <sp:VitalSign>
		   * ..child node
