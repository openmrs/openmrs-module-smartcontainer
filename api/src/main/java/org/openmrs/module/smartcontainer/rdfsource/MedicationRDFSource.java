...
public class MedicationRDFSource extends RDFSource {
	...
	private String getRDF(List<DrugOrder> drugs) throws IOException {
		...
		for (DrugOrder oder : drugs) {
			...
			graph.writeStatement(medicationNode, drugName,
					codedValue(graph, oder.getDrug()));
			//
			if (oder.getStartDate() != null) {
				URI startDate = factory.createURI(sp, "startDate");
				Value startDateVal = factory.createLiteral(date(oder
						.getStartDate()));
			...
			if ((oder.getQuantity() != null) && (oder.getUnits() != null)) {
				...
				graph.writeStatement(
						medicationNode,
						quantity,
						valueAndUnit(oder.getQuantity().toString(),
								oder.getUnits(), graph));
			}
			//
			if ((oder.getFrequency() != null) && (oder.getUnits() != null)) {
				...
				graph.writeStatement(
						medicationNode,
						frequency,
						valueAndUnit(oder.getFrequency().toString(),
								oder.getUnits(), graph));
	...
	private Value valueAndUnit(String quantity, String units, RdfXmlWriter graph)
			throws IOException {
